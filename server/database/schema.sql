...
CREATE TABLE languages (
    ...
    language_code VARCHAR(10) NOT NULL UNIQUE, -- e.g., 'en', 'et', 'ru', 'fi'
    language_name VARCHAR(100) NOT NULL, -- e.g., 'English', 'Eesti', 'Русский'
...
CREATE TABLE sets (
    ...
    category VARCHAR(100), -- e.g., 'electronics', 'woodwork', 'games'
    ...
    teacher_manual_pdf TEXT, -- Path to teacher manual PDF
    student_manual_pdf TEXT, -- Path to student manual PDF
    production_manual_pdf TEXT, -- Path to production manual PDF
    drawing_pdf TEXT, -- Path to drawing PDF
    active BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE parts (
    ...
    part_number VARCHAR(100) UNIQUE, -- SKU or internal code
    category VARCHAR(100), -- e.g., 'electronic', 'wood', 'fastener', 'tool'
    unit_of_measure VARCHAR(50), -- e.g., 'piece', 'meter', 'gram'
    ...
    image_url TEXT,
    instruction_pdf TEXT, -- Path to instruction PDF
    drawing_pdf TEXT, -- Path to drawing PDF
    assembly_notes TEXT, -- Assembly instructions
    safety_notes TEXT, -- Safety instructions
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE instruction_types (
    ...
    type_code VARCHAR(50) NOT NULL UNIQUE, -- e.g., 'preparation', 'teacher', 'student'
...
CREATE TABLE media_files (
    ...
    file_type VARCHAR(50), -- e.g., 'image', 'pdf', 'video'
...
CREATE TABLE set_media (
    ...
    set_id INTEGER REFERENCES sets(set_id) ON DELETE CASCADE,
    media_id INTEGER REFERENCES media_files(media_id) ON DELETE CASCADE,
    media_category VARCHAR(100), -- e.g., 'product_photo', 'drawing', 'step_photo'
    display_order INTEGER,
